...
CREATE TABLE IF NOT EXISTS Лекарства (
    ...
    CONSTRAINT check_application CHECK (Способ_применения IN
('внутреннее', 'наружное', 'для смешивания')),
...
(Статус IN ('ожидание компонентов', 'готов к производству',
...
CREATE TABLE IF NOT EXISTS Заявки_на_пополнение_компонентов (
                                                                Component_request_id INT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS Заявки_на_пополнение_готовых_лекарств (
                                                                     Medicine_request_id INT PRIMARY KEY,
                                                                     Quantity INT NOT NULL CHECK (Quantity > 0),
    Status VARCHAR(100) NOT NULL,
    Supplier_id INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS Готовые_лекарства (
    ...
    Medicine_request_id INT,

    CONSTRAINT FK_ready_medicine FOREIGN KEY (Medicine_id)
    REFERENCES Лекарства(Medicine_id)
    ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_ready_request FOREIGN KEY (Medicine_request_id)
    REFERENCES Заявки_на_пополнение_готовых_лекарств(Medicine_request_id)
...
CREATE TABLE IF NOT EXISTS Изготавливаемые_лекарства (
    ...
    CONSTRAINT check_compounded_type CHECK (Тип_препарата IN
('микстура', 'мазь', 'раствор', 'настойка', 'порошок'))
    );
